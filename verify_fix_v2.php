...
echo "Created temp user: {$tempUser->id}\n";

// 2. Create a request with this user
...
$req = PriceChangeRequest::create([
    ...
    'requested_by' => $tempUser->id,
    'current_price' => 1000,
    'requested_price' => 2000,
    'reason' => 'test deleted requester',
    'status' => 'pending'
]);
echo "Created request ID: {$req->id}\n";

// 3. Delete the user to simulate "missing requester"
$tempUser->delete();
echo "Deleted temp user.\n";

// Reload request to ensure relation is fresh
$req = PriceChangeRequest::find($req->id);
if (!$req->requester) {
    echo "Confirmed: Request has no requester object.\n";
} else {
    echo "Warning: Requester still exists? " . $req->requester->id . "\n";
}
