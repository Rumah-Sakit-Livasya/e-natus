...
class ProjectRequestResource extends Resource
{
    ...
    public static function form(Form $form): Form
    {
        ...
        return $form
            ->schema([
                Select::make('client_id')->label('Klien')->options(Client::pluck('name', 'id')->toArray())->searchable()->reactive()->afterStateUpdated(function ($state, callable $set) {
                    $client = Client::with('region')->find($state);
                    if ($client) {
                        $set('pic', $client->pic);
                        $set('lokasi', $client->region?->name);
                    }
                })->required()->createOptionForm([TextInput::make('name')->label('Nama Klien')->required(), TextInput::make('pic')->label('PIC')->required(), Select::make('region_id')->label('Wilayah')->options(\App\Models\Region::pluck('name', 'id'))->searchable()->nullable(), TextInput::make('phone')->label('Nomor Telepon')->tel(), TextInput::make('email')->label('Email')->email(),])->createOptionUsing(function (array $data) {
                    return Client::create(['name' => $data['name'], 'pic' => $data['pic'], 'region_id' => $data['region_id'] ?? null, 'phone' => $data['phone'] ?? null, 'email' => $data['email'] ?? null,])->id;
                }),
                TextInput::make('name')->label('Nama Proyek')->required(),
                Select::make('employee_ids')->label('PIC')->multiple()->searchable()->preload()->options(\App\Models\Employee::join('users', 'employees.user_id', '=', 'users.id')->pluck('users.name', 'employees.id')->toArray())->required()->createOptionForm([TextInput::make('name')->label('Nama Pegawai')->required(),])->createOptionUsing(function (array $data) {
                    return \App\Models\SDM::create(['name' => $data['name'],])->id;
                }),
                Select::make('sdm_ids')->label('SDM')->multiple()->searchable()->preload()->options(\App\Models\SDM::pluck('name', 'id')->toArray())->required()->createOptionForm([TextInput::make('name')->label('Nama SDM')->required(),])->createOptionUsing(function (array $data) {
                    return \App\Models\SDM::create(['name' => $data['name'],])->id;
                }),
                TextInput::make('jumlah')->label('Jumlah Peserta')->numeric()->required(),
                TextInput::make('lokasi')->label('Lokasi')->required(),
                DatePicker::make('start_period')->label('Periode Mulai')->required()->minDate(Carbon::today())->reactive()->afterStateUpdated(function (Get $get, Set $set) {
                    self::updateAllTotalsInRepeater($get, $set);
                }),
                DatePicker::make('end_period')->label('Periode Selesai')->required()->minDate(Carbon::today())->reactive()->afterStateUpdated(function (Get $get, Set $set) {
                    self::updateAllTotalsInRepeater($get, $set);
                }),
                ...
                Repeater::make('rencanaAnggaranBiaya')
                    ...
                    ->relationship()
                    ->schema([
                        TextInput::make('description')->label('Deskripsi')->required()->placeholder('Misalnya: Sewa AC Standing'),
                        TextInput::make('qty_aset')->label('Jumlah')->numeric()->required()->live()->debounce(500)->afterStateUpdated(fn(Get $get, Set $set) => self::updateRowTotal($get, $set)),
    ...
    {
        return $table->columns([TextColumn::make('name')->label('Nama Proyek')->searchable()->sortable(), TextColumn::make('client.name')->label('Klien')->sortable(), TextColumn::make('pic')->label('PIC'), TextColumn::make('employee_ids')->label('Employee')->formatStateUsing(function ($state) {
            ...
            return implode(', ', $names);
        }), TextColumn::make('jumlah')->label('Jumlah Peserta')->numeric(), TextColumn::make('lokasi')->label('Lokasi'), TextColumn::make('user.name')->label('Dibuat oleh')->sortable(), TextColumn::make('status')->label('Status')->badge()->color(fn(string $state): string => match ($state) {
            'pending' => 'warning',
            'approved' => 'success',
            'rejected' => 'danger',
            'done' => 'success',
        })->sortable(),])->defaultSort('id', 'desc')->filters([])->actions([Action::make('createRealisasiRab')->label('Tambah Realisasi RAB')->icon('heroicon-o-plus-circle')->modalHeading('Tambah Realisasi RAB')->hidden()->form(function ($record) {
            return [Select::make('rencana_anggaran_biaya_id')->label('Item RAB')->options($record->rencanaAnggaranBiaya()->pluck('description', 'id')->toArray())->searchable()->required(), TextInput::make('description')->label('Deskripsi')->required(), TextInput::make('qty')->label('Jumlah')->numeric(), TextInput::make('harga')->label('Harga')->numeric()->mask(RawJs::make('$money($input)'))->stripCharacters(',')->dehydrateStateUsing(fn(?string $state): ?string => $state ? preg_replace('/[^\d]/', '', $state) : null), DatePicker::make('tanggal_realisasi')->label('Tanggal Realisasi'), Select::make('status')->label('Status')->options(['draft' => 'Draft', 'approved' => 'Approved', 'rejected' => 'Rejected', 'done' => 'Done'])->default('draft')->required(), Textarea::make('keterangan')->label('Keterangan')->rows(2)->nullable(),];
            ...
            Notification::make()->title('Realisasi berhasil ditambahkan')->success()->send();
        }), Action::make('viewAssets')->icon('heroicon-o-eye')->tooltip('Lihat Aset')->label('Aset')->modalHeading('Daftar Aset')->modalSubheading('Berikut adalah aset yang terkait dengan project ini.')->modalButton('Tutup')->action(fn() => null)->modalContent(fn($record) => new HtmlString(\Livewire\Livewire::mount('project-asset-table', ['assetIds' => $record->asset_ids ?? [],])->html())), Action::make('viewRAB')->icon('heroicon-o-document-text')->tooltip('Lihat Rencana Anggaran Biaya')->label('RAB')->modalHeading('Rencana Anggaran Biaya')->modalSubmitAction(false)->modalCancelActionLabel('Tutup')->modalWidth('4xl')->visible(fn($record) => $record->rencanaAnggaranBiaya()->exists())->modalContent(function ($record) {
        ...
        }), Action::make('createClosingRab')->label('Buat Draft RAB Closing')->color('warning')->action(function (ProjectRequest $record, $livewire) {
            if ($record->rabClosing()->exists()) {
                Notification::make()->title('Informasi')->body('Draft RAB Closing sudah ada. Silakan edit dari sana.')->info()->send();
                $rabClosingId = $record->rabClosing->id;
                return redirect()->to(RabClosingResource::getUrl('edit', ['record' => $rabClosingId]));
            }
            try {
                DB::beginTransaction();
                $rabClosing = $record->rabClosing()->create(['closing_date' => now(), 'status' => 'draft',]);
                foreach ($record->rencanaAnggaranBiaya as $itemAwal) {
                    $rabClosing->items()->create(['description' => $itemAwal->description, 'qty' => $itemAwal->qty_aset, 'satuan' => '', 'harga_satuan' => $itemAwal->harga_sewa, 'total_anggaran' => $itemAwal->total,]);
                }
                DB::commit();
                Notification::make()->title('Berhasil')->body('Draft RAB Closing berhasil dibuat. Anda akan diarahkan untuk mengedit.')->success()->send();
        ...
        })->visible(fn(ProjectRequest $record): bool => $record->status !== 'done'), Tables\Actions\EditAction::make(),])->bulkActions([Tables\Actions\DeleteBulkAction::make(),]);
    }
    ...
    {
        return ['index' => Pages\ListProjectRequests::route('/'), 'create' => Pages\CreateProjectRequest::route('/create'), 'edit' => Pages\EditProjectRequest::route('/{record}/edit'),];
    }
