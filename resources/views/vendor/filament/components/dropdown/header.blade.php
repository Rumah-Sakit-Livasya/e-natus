...
<{{ $tag }}
    ...
    @if (filled($icon))
        <x-filament::icon :icon="$icon" @class([
            ...
            \Filament\Support\get_color_css_variables($color, shades: [400, 500], alias: 'dropdown.header.icon') => $color !== 'gray',
        ]) />
    @endif
    ...
    <span @class([
        ...
        \Filament\Support\get_color_css_variables($color, shades: [400, 600], alias: 'dropdown.header.label') => $color !== 'gray',
    ])>
        {{ $slot }}
    </span>
    </{{ $tag }}>
